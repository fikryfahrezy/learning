...
class Game {
  ...
  Word guess(String guess) {
    final normalizedGuess = _validateGuess(guess);
    final result = _evaluateGuess(normalizedGuess);
  ...
  bool isLegalGuess(String guess) {
    ...
    if (normalizedGuess.length != Word.wordLength) return false;
    return allLegalGuesses.contains(normalizedGuess);
  }

  /// Evaluates [guess] against the hidden word without advancing the game.
  Word matchGuessOnly(String guess) {
    final normalizedGuess = _validateGuess(guess);
    return _evaluateGuess(normalizedGuess);
  }
  ...
  Word _evaluateGuess(String guess) =>
      Word.fromString(guess).evaluateGuess(_wordToGuess);

  String _validateGuess(String guess) {
    final normalizedGuess = _normalizeGuess(guess);

    if (normalizedGuess.length != Word.wordLength) {
      throw ArgumentError.value(
        guess,
        'guess',
        'Guess must be exactly ${Word.wordLength} letters.',
      );
    }
    ...
    if (!isLegalGuess(normalizedGuess)) {
      throw ArgumentError.value(
        guess,
        'guess',
        'Guess is not in the current word list.',
      );
    }
  ...
  static String _normalizeGuess(String guess) => guess.trim().toLowerCase();
}

/// A five-letter word made up of [Letter]s, each tracking its [HitType].
class Word with IterableMixin<Letter> {
  /// The fixed length of every playable word in Birdle.
  ...
  factory Word.fromString(String guess) {
    if (guess.length != wordLength) {
      throw ArgumentError.value(
        guess,
        'guess',
        'Must be exactly $wordLength characters long.',
      );
    }
